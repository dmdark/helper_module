...
function _seoInitializeJQuery(callback){
   if(typeof(_seoJQuery) == 'undefined' || !_seoJQuery){
      if(!_seoInitializeJQuery.included){
         ...
         document.write('<scr' + 'ipt type="text/javascript" src="/_seo/modules/jquery.js"></scr' + 'ipt>');
      ...
      if(!$.fn.formValidator){
         $.fn.formValidator = function (){
            ...
            form.find('[data-validate="true"]').each(function (){
               ...
               if(field.data("validate")){
                  field.on('focusout', function (){
                     ...
                     $.ajax({
                        url: '/_seo/modules/forms/inputvalidator.php', data: postData, type: 'POST', success: function (text){
                           if(text != 1){
            ...
            form.on('submit', function (e){
               e.preventDefault();
               $.ajax({
                  url: '/_seo/modules/forms/formvalidator.php', data: form.serialize(), dataType: 'json', type: 'POST', cache: false, success: function (jsonData){
