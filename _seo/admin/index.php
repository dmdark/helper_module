	...
	<div class="container">
		...
		<div class="row row-item row-first-{{ $first }}" ng-class="" ng-repeat="item in items">
			<div class="panel" id="url_number_{{ $index + 1 }}">
				...
				<?php
				$additionalTags = array_key_exists('additionalTags',$config['adminConfig'])? $config['adminConfig']['additionalTags'] : '';
				...
				<div class="input-group" style="margin-top: 7px;">
					...
					<input type="text" class="form-control" placeholder="<?php echo "Замена крошки. Пример: [Купить|Арендовать] {t} [недорого|дешево]"; ?>" ng-model="item.crumbskw">
				</div>
				<div style="text-align: right;" ng-show="item.rememberCache">
					<a href="" ng-click="item.showCache = !item.showCache">show remember cache</a>
				</div>
				<code ng-show="item.showCache" ng-bind-html="item.rememberCache"></code>
			</div>
		</div>
	</div>
<?php endif; ?>
